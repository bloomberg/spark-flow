...
//    println(s"input: ${I.input.getSignature}")
//
//    println(s"string: ${hashClass(S)}")
//
//    println(s"effect: ${hashClass(E.playWithValues)}")

    println(getFieldObjects(HashingSample.result))
    println(s"result: ${HashingSample.result.getSignature}")

//    result.getRDD(sc).collect().foreach(println)
  }
