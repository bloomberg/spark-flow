...
//    new RDDTransformDC((rdd: RDD[(K,V)]) => rdd.mapValues(f), self, f, Seq("mapValues"))
//  }
//
//  def flatMapValues[U](f: V => TraversableOnce[U])(implicit kUEncoder: Encoder[(K,U)]): DC[(K, U)] = {
//    new RDDTransformDC((rdd: RDD[(K,V)]) => rdd.flatMapValues(f), self, f, Seq("flatMapValues"))
//  }
//
...
//  def keys: DC[K] = {
//    new RDDTransformDC((rdd: RDD[(K, V)]) => rdd.keys, self, Seq("keys"))
//  }
//
//  def values: DC[V] = {
//    new RDDTransformDC((rdd: RDD[(K, V)]) => rdd.values, self, Seq("values"))
//  }
//
//  def partitionBy(partitioner: Partitioner): DC[(K,V)] = {
//    new RDDTransformDC((rdd: RDD[(K, V)]) => rdd.partitionBy(partitioner), self, Seq("partitionBy", partitioner.numPartitions.toString))
//  }
//
//  def partitionByKey(): DC[(K,V)] = {
//    new RDDTransformDC((rdd: RDD[(K, V)]) => rdd.partitionBy(new HashPartitioner(rdd.partitions.length)), self, Seq("partitionByKey"))
//  }
//
//  def repartitionAndSortWithinPartitions(partitioner: Partitioner): DC[(K, V)] = {
//    new RDDTransformDC((rdd: RDD[(K, V)]) => rdd.repartitionAndSortWithinPartitions(partitioner), self, Seq("repartitionAndSortWithinPartitions", partitioner.numPartitions.toString))
//  }
//
//  def sortByKey(ascending: Boolean = true): DC[(K,V)] = {
//    new RDDTransformDC((rdd: RDD[(K,V)]) => rdd.sortByKey(ascending), self, Seq("sortByKey", ascending.toString))
//  }
//
//  def sortByKey(ascending: Boolean, numPartitions: Int): DC[(K,V)] = {
//    new RDDTransformDC((rdd: RDD[(K,V)]) => rdd.sortByKey(ascending, numPartitions), self, Seq("sortByKey", ascending.toString, numPartitions.toString))
//  }
